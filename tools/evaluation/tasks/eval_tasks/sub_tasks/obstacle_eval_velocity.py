...
class ObstacleEvalVelocity:
    ...
    @staticmethod
    def acquire_velocity(instant):
        utm_abs_velocity_x = instant.get_utm_abs_vel_x() \
            if hasattr(instant, "get_utm_abs_vel_x") and instant.get_utm_abs_vel_x() is not None else None
        utm_abs_velocity_y = instant.get_utm_abs_vel_y() \
            if hasattr(instant, "get_utm_abs_vel_y") and instant.get_utm_abs_vel_x() is not None else None
        velodyne_abs_velocity_x = instant.get_lidar_abs_vel_x() \
            if hasattr(instant, "get_lidar_abs_vel_x") and instant.get_lidar_abs_vel_x() is not None else None
        velodyne_abs_velocity_y = instant.get_lidar_abs_vel_y() \
            if hasattr(instant, "get_lidar_abs_vel_y") and instant.get_lidar_abs_vel_y() is not None else None
        return {"utm_abs_velocity_x": utm_abs_velocity_x,
                "utm_abs_velocity_y": utm_abs_velocity_y,
                "velodyne_abs_velocity_x": velodyne_abs_velocity_x,
                "velodyne_abs_velocity_y": velodyne_abs_velocity_y}
    ...
    def collect_data(self):
        ...
        for frame_seq, match_pair in enumerate(self.match_pair_list):
            gt_info = self.acquire_velocity(match_pair.gt_instant)
            ...
            for type_name, instant, _ in match_pair.pred_instants:
                pred_info = self.acquire_velocity(instant)
    ...
    def evaluate_result(self, table_for_evaluate):
        target_attrs = [["utm_abs_velocity_x", "utm_abs_velocity_y"],
                        ["velodyne_abs_velocity_x", "velodyne_abs_velocity_y"]]
    ...
    def save_raw_data(self, table_for_record):
        table_for_record.to_csv(os.path.join(self.out_path, "velocity_raw_data.csv"))
