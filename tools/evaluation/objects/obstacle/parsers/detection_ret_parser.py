...
from .attribute_tool import Attr, ts_round, ObstacleEnum, init_polygon, offset_z
...
def parse_object_list(data_list):
    """
    For evaluation of both onboard and offboard lidar detection in the format of list of json files.
    Args:
        data_list: dict of boxes info by frames.
    Returns:
        DataFrame saves each single obj as one row.
    """
    extract_data = {Attr.ts: [],
                    ...
                    Attr.category: [],
                    Attr.is_key_obj: []}
    for frame_seq, ts in enumerate(data_list):
        frame_data = data_list[ts]['pred']
        try:
            ts = float(ts)
        except ValueError:
            ts = ts
