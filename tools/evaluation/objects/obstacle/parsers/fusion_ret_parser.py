...
def parser(data_path):
    """
        ...
        Args:
            data_path: Absolute path of json files which contains "fusion result" of obstacles.
        Returns:
    ...
    default_value = None
    default_scalar = {"x": default_value, "y": default_value, "z": default_value}
    for frame_seq, file_name in enumerate(sorted(os.listdir(data_path))):
        ...
        for obj in frame_data:
            ...
            utm_center = obj["utm_position"] if "utm_position" in obj else default_scalar
            utm_x, utm_y, utm_z = utm_center["x"], utm_center["y"], utm_center["z"]
            utm_velocity = obj["utm_velocity"] if "utm_velocity" in obj else default_scalar
            utm_vx, utm_vy, utm_vz = utm_velocity["x"], utm_velocity["y"], utm_velocity["z"]
            utm_acc = obj["utm_acceleration"] if "utm_acceleration" in obj else default_scalar
            utm_ax, utm_ay, utm_az = utm_acc["x"], utm_acc["y"], utm_acc["z"]
            utm_yaw = obj["utm_yaw"] if "utm_yaw" in obj else default_value

            extract_data[Attr.ts].append(ts)
            extract_data[Attr.frame_seq].append(frame_seq)
            measure_ts = ts_round(obj["measure_timestamp"]) if "measure_timestamp" in obj else ts
            extract_data[Attr.measure_ts].append(measure_ts)
