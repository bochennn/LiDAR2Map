...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, location, ego2global_translation, ego2global_rotation):
        ...
        vectors = []
        for line_type, vects in line_vector_dict.items():   # road_divider, lane_divider
            for line, length in vects:
                vectors.append((line.astype(float), length, 'divider'))

        for ped_line, length in ped_vector_list:
            vectors.append((ped_line.astype(float), length, 'ped_crossing'))

        for contour, length in poly_bound_list:
            vectors.append((contour.astype(float), length, 'boundary'))
    ...
    def poly_geoms_to_vectors(self, polygon_geom):
        ...
        if union_segments.geom_type == 'MultiLineString':
            print(type(union_segments), len(union_segments.geoms), str(union_segments))
    ...
    def ped_geoms_to_vectors(self, ped_geom):
        ...
        if union_ped.geom_type == 'LineString':
            return results
        elif union_ped.geom_type == 'MultiLineString':
            union_ped = MultiPolygon([union_ped.convex_hull])
        elif union_ped.geom_type != 'MultiPolygon':
            union_ped = MultiPolygon([union_ped])

        for ped_poly in union_ped.geoms:
