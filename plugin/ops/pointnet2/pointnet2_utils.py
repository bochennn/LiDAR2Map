...
from . import pointnet2_ext as pointnet2
...
def sector_fps(points, num_sampled_points, num_sectors):
    ...
    point_angles = torch.atan2(points[:, 1], points[:, 0]) + math.pi
...
class QueryAndGroup(BaseModule):
    ...
    def forward(self,
        ...
        # idx: (M1 + M2 ..., nsample)
        idx, empty_ball_mask = ball_query(self.radius, self.sample_nums,
                                          xyz, xyz_batch_cnt,
                                          new_xyz, new_xyz_batch_cnt)

        grouped_xyz = grouping_operation(xyz, xyz_batch_cnt,
                                         idx, new_xyz_batch_cnt)  # (M1 + M2, 3, nsample)
        grouped_xyz -= new_xyz.unsqueeze(-1)
        grouped_xyz[empty_ball_mask] = 0

        if features is not None:
            grouped_features = grouping_operation(
                features, xyz_batch_cnt, idx, new_xyz_batch_cnt)  # (M1 + M2, C, nsample)
...
class FarthestPointSampling(Function):
    @staticmethod
    def forward(ctx, xyz: torch.Tensor, npoint: int) -> torch.IntTensor:
        """
        Args:
            ctx:
